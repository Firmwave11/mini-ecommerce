...
func LoadMultipleDatabase() {
	...
	listDb := viper.GetString("productservice.db.lists")
	db := strings.Split(listDb, ",")

	// slice db name connection into array string
	listDbName := viper.GetString("productservice.db.name")
...
func LoadMigrationsDatabase() {
	// connection strings
	connstrings := viper.GetString("productservice.db.migrations.connstrings")

	// folder migration sql
	folder := viper.GetString("productservice.db.migrations.folder")

...
func LoadMultipleDatabase() {
	...
	listDb := viper.GetString("warehouseservice.db.lists")
	db := strings.Split(listDb, ",")

	// slice db name connection into array string
	listDbName := viper.GetString("warehouseservice.db.name")
...
func LoadMigrationsDatabase() {
	// connection strings
	connstrings := viper.GetString("warehouseservice.db.migrations.connstrings")

	// folder migration sql
	folder := viper.GetString("warehouseservice.db.migrations.folder")

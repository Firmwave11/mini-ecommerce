...
func LoadMultipleDatabase() {
	...
	listDb := viper.GetString("orderservice.db.lists")
	db := strings.Split(listDb, ",")

	// slice db name connection into array string
	listDbName := viper.GetString("orderservice.db.name")
...
func LoadMigrationsDatabase() {
	// connection strings
	connstrings := viper.GetString("orderservice.db.migrations.connstrings")

	// folder migration sql
	folder := viper.GetString("orderservice.db.migrations.folder")

...
func LoadMultipleDatabase() {
	...
	listDb := viper.GetString("shopservice.db.lists")
	db := strings.Split(listDb, ",")

	// slice db name connection into array string
	listDbName := viper.GetString("shopservice.db.name")
...
func LoadMigrationsDatabase() {
	// connection strings
	connstrings := viper.GetString("shopservice.db.migrations.connstrings")

	// folder migration sql
	folder := viper.GetString("shopservice.db.migrations.folder")
